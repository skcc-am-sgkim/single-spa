...
import { MenuList } from "@szhsin/react-menu/dist/es/components/MenuList.js";
import { jsxs, jsx } from "react/jsx-runtime";
import { withHovering } from "@szhsin/react-menu/dist/es/utils/withHovering.js";
import {
  menuPropTypes,
  uncontrolledMenuPropTypes,
  stylePropTypes,
} from "@szhsin/react-menu/dist/es/utils/propTypes.js";
import { useMenuStateAndFocus } from "@szhsin/react-menu/dist/es/hooks/useMenuStateAndFocus.js";
import { useItemEffect } from "@szhsin/react-menu/dist/es/hooks/useItemEffect.js";
import { useMenuChange } from "@szhsin/react-menu/dist/es/hooks/useMenuChange.js";
import { useBEM } from "@szhsin/react-menu/dist/es/hooks/useBEM.js";
import {
  SettingsContext,
  MenuListContext,
  MenuListItemContext,
  ...
  Keys,
  FocusPositions,
} from "@szhsin/react-menu/dist/es/utils/constants.js";
import { useCombinedRef } from "@szhsin/react-menu/dist/es/hooks/useCombinedRef.js";
import {
  mergeProps,
  commonProps,
  safeCall,
  isMenuOpen,
  batchedUpdates,
} from "@szhsin/react-menu/dist/es/utils/utils.js";

const SubMenu = /*#__PURE__*/ withHovering(
  "SubMenu",
  function SubMenu({
    ...
    const { isParentOpen, submenuCtx, dispatch, updateItems } =
    ...
    const setHover = () =>
      !isHovering &&
      !isDisabled &&
      dispatch(HoverActionTypes.SET, itemRef.current);
    const delayOpen = (delay) => {
      setHover();
      if (!openTrigger)
        timerId.v = setTimeout(
          () => batchedUpdates(openMenu),
          Math.max(delay, 0)
        );
    };
    ...
    useEffect(() => {
      /* 마지막 서브 메뉴 호버 않했을때도 메뉴 닫히지 않게 변경 */
    ...
    const mergedItemProps = mergeProps(
      {
        ...
        onClick: () => openTrigger !== "none" && toggleMenu(!isOpen),
      },
      restItemProps
    );
    const getMenuList = () => {
      const menuList = /*#__PURE__*/ jsx(MenuList, {
        ...
        direction:
          direction ||
          (parentDir === "right" || parentDir === "left" ? parentDir : "right"),
        parentScrollingRef: isPortal && parentMenuRef,
        isDisabled: isDisabled,
      });
      const container = rootMenuRef.current;
      return isPortal && container
        ? /*#__PURE__*/ createPortal(menuList, container)
        : menuList;
    };
